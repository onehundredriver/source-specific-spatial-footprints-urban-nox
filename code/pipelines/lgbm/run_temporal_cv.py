# -*- coding: utf-8 -*-
"""
Configuration-driven LightGBM temporal 5-fold cross-validation.
...
python code/pipelines/lgbm/run_temporal_cv.py --config code/config/lgbm_temporal_1hour.json
python code/pipelines/lgbm/run_temporal_cv.py --config code/config/lgbm_temporal_24hour.json
python code/pipelines/lgbm/run_temporal_cv.py --config code/config/lgbm_temporal_168hour.json
python code/pipelines/lgbm/run_temporal_cv.py --config code/config/lgbm_temporal_31day.json
"""
...
# Model note
# =============================================================================
# This file was generated from the XGBoost temporal pipeline.
# Only the estimator and model-specific default parameters were changed.
# Bayesian optimization is intentionally disabled by default for this public wrapper.
# Generated at: 2026-05-08 23:18:42
...
# code/pipelines/lgbm/run_temporal_cv.py
...
def load_and_preprocess_data(
    ...
    This function follows the logic of the original temporal LightGBM scripts:
...
# =============================================================================
# LightGBM optimization and training
# =============================================================================

def lgbm_parameter_selection(
    ...
    fixed_params = cfg["lgbm_fixed_params"]

    if not bo_cfg.get("enabled", True):
        print("[INFO] Bayesian optimization disabled. Using default LightGBM parameters.")
        params = dict(fixed_params)
        params.update(cfg.get("default_lgbm_params_when_bo_disabled", {'n_estimators': 300,
 'max_depth': -1,
 'learning_rate': 0.05,
 'num_leaves': 31,
 'subsample': 0.8,
 ...
 'reg_lambda': 0.0}))
    ...
    raise NotImplementedError(
        "LightGBM Bayesian optimization is not included in this public wrapper. "
...
def run_temporal_cv(cfg: dict) -> None:
    """
    Main workflow for temporal 5-fold LightGBM and SHAP extraction.
    """
    ...
    best_params = lgbm_parameter_selection(
    ...
    for fold_idx in range(1, n_folds + 1):
        ...
        perm_res = permutation_importance(
            ...
            y_train,
            n_repeats=5,
            random_state=int(cfg["lgbm_fixed_params"].get("random_state", 42)),
            n_jobs=-1,
        )
    ...
    summary_str = (
        f"\n{'=' * 20} 5-Fold Temporal CV 平均指标汇总 (LightGBM) {'=' * 20}\n"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run LightGBM temporal 5-fold CV from a JSON config."
    )
    parser.add_argument(
        "--config",
        required=True,
