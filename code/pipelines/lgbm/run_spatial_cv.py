# -*- coding: utf-8 -*-
"""
Configuration-driven LightGBM spatial 5-fold cross-validation.

Example commands, run from repository root:

python code/pipelines/lgbm/run_spatial_cv.py --config code/config/lgbm_spatial_1hour.json
python code/pipelines/lgbm/run_spatial_cv.py --config code/config/lgbm_spatial_24hour.json
python code/pipelines/lgbm/run_spatial_cv.py --config code/config/lgbm_spatial_168hour.json
python code/pipelines/lgbm/run_spatial_cv.py --config code/config/lgbm_spatial_31day.json
"""
...
# code/pipelines/lgbm/run_spatial_cv.py
...
def lgbm_parameter_selection(X_train: pd.DataFrame, y_train: pd.Series, cfg: dict) -> dict:
    """
 ...
 'reg_lambda': 0.0}))
    ...
    def lgbm_crossval(n_estimators, max_depth, learning_rate, subsample,
...
def run_spatial_cv(cfg: dict) -> None:
    """
    Main workflow for spatial 5-fold LightGBM and SHAP extraction.
    """
    ...
    best_params = lgbm_parameter_selection(X_global_scaled, y_global, cfg)
    ...
    summary_str = (
        f"\n{'=' * 20} 5-Fold CV 平均指标汇总 (LightGBM) {'=' * 20}\n"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run LightGBM spatial 5-fold CV from a JSON config.")
    parser.add_argument(
        "--config",
        required=True,
