# -*- coding: utf-8 -*-
"""
Configuration-driven Random Forest temporal 5-fold cross-validation.
...
python code/pipelines/rf/run_temporal_cv.py --config code/config/rf_temporal_1hour.json
python code/pipelines/rf/run_temporal_cv.py --config code/config/rf_temporal_24hour.json
python code/pipelines/rf/run_temporal_cv.py --config code/config/rf_temporal_168hour.json
python code/pipelines/rf/run_temporal_cv.py --config code/config/rf_temporal_31day.json
"""
...
# code/pipelines/rf/run_temporal_cv.py
...
def load_and_preprocess_data(
    ...
    This function follows the logic of the original temporal Random Forest scripts:
...
def run_temporal_cv(cfg: dict) -> None:
    """
    Main workflow for temporal 5-fold Random Forest and SHAP extraction.
    """
    ...
    best_params = rf_parameter_selection(
    ...
    summary_str = (
        f"\n{'=' * 20} 5-Fold Temporal CV 平均指标汇总 (Random Forest) {'=' * 20}\n"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Random Forest temporal 5-fold CV from a JSON config."
    )
    parser.add_argument(
        "--config",
        required=True,
