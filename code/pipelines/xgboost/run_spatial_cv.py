# -*- coding: utf-8 -*-
"""
Configuration-driven XGBoost spatial 5-fold cross-validation.
...
python code/pipelines/xgboost/run_spatial_cv.py --config code/config/xgboost_spatial_24hour.json
python code/pipelines/xgboost/run_spatial_cv.py --config code/config/xgboost_spatial_168hour.json
python code/pipelines/xgboost/run_spatial_cv.py --config code/config/xgboost_spatial_31day.json
"""
...
# code/pipelines/xgboost/run_spatial_cv.py
...
def xgb_bayesian_optimization(X_train: pd.DataFrame, y_train: pd.Series, cfg: dict) -> dict:
    """
...
def run_spatial_cv(cfg: dict) -> None:
    """
    Main workflow for spatial 5-fold XGBoost and SHAP extraction.
    """
    ...
    best_params = xgb_bayesian_optimization(X_global_scaled, y_global, cfg)
    ...
    summary_str = (
        f"\n{'=' * 20} 5-Fold CV 平均指标汇总 (XGBoost) {'=' * 20}\n"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run XGBoost spatial 5-fold CV from a JSON config.")
    parser.add_argument(
        "--config",
        required=True,
